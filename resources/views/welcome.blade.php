...
{{-- Loading Type Badge --}}
<div class="loading-badge" style="background:rgba(81,207,102,.12);border-color:rgba(81,207,102,.38);color:#8ce99a;">LAZY LOADING</div>

{{-- Loading Stats Badge --}}
<div class="perf-badge">
  <div class="perf-badge-title">Loading Stats</div>
  <div>Loaded: <span id="loaded-count">0</span> / <span id="total-count">0</span></div>
  <div class="perf-badge-note">Intersection Observer API</div>
</div>
...
<style>
  .perf-badge {
    ...
    background: rgba(7,22,40,.95); border: 1px solid rgba(81,207,102,.3);
    color: var(--white); padding: 14px 18px; border-radius: 10px;
    font-family: 'Space Grotesk', sans-serif; font-size: .75rem;
    z-index: 1000; backdrop-filter: blur(12px); min-width: 200px;
    line-height: 1.8;
  }
  .perf-badge-title { font-weight: 700; color: #8ce99a; margin-bottom: 4px; }
  .perf-badge-note  { font-size: .68rem; color: var(--muted); margin-top: 6px; }
  #loaded-count     { color: #8ce99a; font-weight: 700; }

  /* Lazy skeleton — dark shimmer sebelum gambar dimuat */
  img.lazy:not(.lazy-loaded) {
    background: linear-gradient(90deg, #071628 25%, #0a1f3d 50%, #071628 75%);
  ...
  img.lazy-loaded {
    animation: lazy-fadein .5s ease-in;
  }
    ...
    <div class="profil-inner">
      ...
      <div class="profil-img-col reveal">
        <div class="profil-img-wrap">
          ...
          <img class="lazy img-float"
               src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7"
               data-src="{{ asset('img/gambar2.png') }}"
               alt="Robot hand with shield">
        </div>
      </div>
    ...
    <div class="vm-layout">
      ...
      <div class="vm-img-col reveal delay-2">
        <div class="vm-img-wrap">
          ...
          <img class="lazy img-float-vm"
               src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7"
               data-src="{{ url('img/visimisi.png') }}"
          ...
          <div class="carousel-inner">
            @for($i = 1; $i <= 5; $i++)
              <div class="carousel-item @if($i == 1) active @endif">
                ...
                {{-- Popup dimuat eager karena langsung tampil saat halaman dibuka --}}
...
{{-- LAZY LOADING: Intersection Observer untuk semua img.lazy --}}
<script>
document.addEventListener('DOMContentLoaded', function () {
  const lazyImages   = document.querySelectorAll('img.lazy');
  const loadedEl     = document.getElementById('loaded-count');
  const totalEl      = document.getElementById('total-count');
  let   loadedCount  = 0;

  totalEl.textContent = lazyImages.length;
  ...
  const imgObserver = new IntersectionObserver(function (entries, observer) {
    entries.forEach(function (entry) {
      ...
      img.addEventListener('load', function () {
        ...
        loadedCount++;
        loadedEl.textContent = loadedCount;
        console.log('🟢 Lazy loaded [' + loadedCount + '/' + lazyImages.length + ']: ' + img.dataset.src);
      });
  ...
  console.log('🟢 LAZY LOADING dengan Intersection Observer');
  console.log('Teknik: data-src + class="lazy", trigger 100px sebelum viewport');
